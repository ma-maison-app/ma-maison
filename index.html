    ...
    <style>
        ...
        * {
        ...
        .modal {
            ...
            z-index: 9999;  /* INCREASED - ensure modal appears over YouTube player */
    ...
    <script>
        ...
        function loadFromLocalStorage() {
            ...
            try {
                ...
                const localPlaybackPositions = localStorage.getItem('listeningPlaybackPositions');
                ...
                if (localPlaybackPositions) listeningPlaybackPositions = JSON.parse(localPlaybackPositions);
        ...
        async function loadDataFromFirebase(userId) {
            ...
            try {
                ...
                if (userDoc.exists()) {
                    ...
                    if (data.listeningPlaybackPositions) {
                        listeningPlaybackPositions = data.listeningPlaybackPositions;
                        localStorage.setItem('listeningPlaybackPositions', JSON.stringify(listeningPlaybackPositions));
                        console.log('✅ Loaded', Object.keys(listeningPlaybackPositions).length, 'playback positions');
                    }
...
Peux-tu ajouter des timestamps réalistes au début de chaque ligne ? Format: [MM:SS] suivi du texte. Commence à 00:00 et ajoute environ 3-5 secondes entre chaque ligne selon la longueur. Retourne SEULEMENT le texte avec timestamps, sans explication.`;
        ...
        // Store playback positions for each listening item (so you can resume where you left off)
        let listeningPlaybackPositions = JSON.parse(localStorage.getItem('listeningPlaybackPositions') || '{}');
        ...
        window.openListeningPlayer = async function(itemId) {
            ...
            if (videoId) {
                ...
                if (youtubePlayer) {
                    ...
                    // RESTORE PLAYBACK POSITION for existing player
                    if (listeningPlaybackPositions[item.id]) {
                        const savedTime = listeningPlaybackPositions[item.id];
                        console.log(`📍 Restoring playback position: ${savedTime.toFixed(1)}s`);
                        setTimeout(() => {
        ...
        window.closeListeningPlayer = function() {
            ...
            if (currentListeningItem && youtubePlayer && typeof youtubePlayer.getCurrentTime === 'function') {
                try {
        ...
        function onPlayerReady(event) {
            ...
            if (currentListeningItem && listeningPlaybackPositions[currentListeningItem.id]) {
                const savedTime = listeningPlaybackPositions[currentListeningItem.id];
                console.log(`📍 Restoring playback position: ${savedTime.toFixed(1)}s`);
                setTimeout(() => {
                    if (youtubePlayer && typeof youtubePlayer.seekTo === 'function') {
                        youtubePlayer.seekTo(savedTime, true);
